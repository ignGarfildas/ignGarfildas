 ...
 * @name AnimatedStatus
 * @version 1.0
 * @author TeamG | ! Garfildas#8854
 * @description Animuok savo statusa
 */
...
class AnimatedStatus {
	...
	Status_Animate (index = 0) {
		...
		if (this.animation[index] == undefined) {
			BdApi.showToast("Animuotas statusas: Nėra statuso. Nueik į Settings>Plugins, kad pasidarytum animuota statusa!");
			return;
		}
	...
	getSettingsPanel () {
		...
		// timeout
		settings.appendChild(GUI.newLabel("Parašyk kas kiek laiko keisis statusas (Milisekundės)"));
		...
		// Animation
		settings.appendChild(GUI.newLabel('Animacija'));
		...
		let changeEditMode = GUI.newButton("Pakeisk redagavimo režimą");
		...
		let save = GUI.newButton("Išsaugok");
		...
		save.onclick = () => {
			...
			// Show Toast
			BdApi.showToast("Nustatymai išsaugoti!", {type: "sėkmingai"});
